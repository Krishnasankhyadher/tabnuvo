...
export default function Enquiries() {
  ...
  async function fetchEnquiries() {
    try {
      setLoading(true);
      setError(null);
      // backend GET endpoint (singular) is /api/enquiry
        ...
        <div className="max-w-7xl mx-auto">
          <header className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-600">View messages submitted via your enquiry form.</p>
            </div>
                    ...
                    {paged.map((e) => (
                      <tr key={e._id} className="border-t hover:bg-gray-50">
                        ...
                        <td className="p-3 flex gap-2">
                          <button onClick={() => setSelected(e)} className="px-2 py-1 rounded border text-sm">
                            View
                          </button>
                          <button
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {selected.email} • {selected.createdAt ? new Date(selected.createdAt).toLocaleString() : "—"}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => handleDelete(selected._id)} className="px-3 py-1 border rounded text-red-600">
                      Delete
                    </button>
